...
#  
  ...
  summary(data_offer$amt_offer_value) # outliers
  data_offer[, amt_offer_value := DescTools::Winsorize(amt_offer_value, probs = c(0.01, 0.99), na.rm = TRUE)] # winsorize at 1st percentile
  summary(data_offer$amt_offer_value) # outliers 
  
  # Create a "dt_year" variable starting from "dt_offer_start" and check if there are issues with that
  #  tip: lubridate::year()
  data_offer[, dt_year := lubridate::year(dt_offer_start)]
...
# (Bonus Question) Can you think of some unusual circumstances when it comes to count single firms?
  ...
  data_summary = copy(data_offer)
  data_summary = data_summary[, 
             if (.N == 1) .SD, by = c("id_firm_rut", "id_tender", "id_buyer")][
               !is.na(amt_offer_value) & !is.na(dt_year),
                .(`Number of Bidders`  = .N,
                  `Avg Tender Value`   = mean(amt_offer_value, na.rm = TRUE)),
             by = .(id_buyer, id_tender, dt_year)
             ][, 
               .(`Number of Bidders`  = mean(`Number of Bidders`, na.rm = TRUE),
                 `Avg Tender Value`   = mean(`Avg Tender Value`, na.rm = TRUE)),
               by = .(id_buyer, dt_year)
             ][, 
               .(`Number of Bidders`  = mean(`Number of Bidders`, na.rm = TRUE),
                 `Avg Tender Value`   = mean(`Avg Tender Value`, na.rm = TRUE)),
  ...
  print(setorder(data_summary, dt_year))
...
#  
  ...
  data_top10_buyers = copy(data_offer) 
  data_top10_buyers = data_top10_buyers[, 
                                        if (.N == 1) .SD, by = c("id_firm_rut", "id_tender", "id_buyer")][
                                          !is.na(amt_offer_value) & !is.na(dt_year),
                                          .(`Number of Bidders`  = .N),
                                          by = .(id_buyer, id_tender, dt_signature)
                                        ]
  
  # Then you want to merge the data with the payment dataset
  data_top10_buyers = merge(data_top10_buyers, data_payments, by = "id_tender", all.y = TRUE)
  
  # Let's compute the average delay 
  setDT(data_top10_buyers)[, avg_delay := difftime(dt_payment, dt_signature, units="days")]
  
  # We collapse data at the tender level first
  data_top10_buyers = data_top10_buyers[,
    .(`Number of Bidders` = mean(`Number of Bidders`, na.rm = TRUE),
      `Average Payment Delay`  = mean(avg_delay, na.rm = TRUE)),
    by = .(id_buyer, id_tender)
  ]
  
  # Then, we collapse data at the buyer level and extract the first 10
  data_top10_buyers = data_top10_buyers[,
                                        .(`Number of Bidders` = mean(`Number of Bidders`, na.rm = TRUE),
                                          `Average Payment Delay`  = mean(`Average Payment Delay`, na.rm = TRUE)),
                                        by = .(id_buyer)
  ] |> na.omit()
  
  data_top10_buyers = setorder(data_top10_buyers, -`Average Payment Delay`)[1:10]
  
  print(data_top10_buyers)

  # 
  # First, we need to select the first 10 sellers by amount contracted
  data_top10_sell = copy(data_offer) 
  data_top10_sell = data_top10_sell[ind_offer_select == 1,
                                    .(`Total Amount Contracts` = sum(amt_offer_value, na.rm = TRUE)),
                                    by = .(id_firm_rut)
  ]
  data_top10_sell = setorder(data_top10_sell, -`Total Amount Contracts`)[1:10]
  
  # Then we want to merge that with the firm data (we only focus on the size in 2018)
  # 
  data_top10_firm = data_firms
  data_top10_firm = data_top10_firm[id_firm_rut %in% data_top10_sell$id_firm_rut,
                                    .(`Average Number of Employees` = mean(n_employees, na.rm = TRUE)),
                                    by = .(id_firm_rut)
  ]
  
  final_tab = merge.data.table(data_top10_sell, data_top10_firm, by = c("id_firm_rut"))
  
  print(setorder(final_tab, -`Total Amount Contracts`))
